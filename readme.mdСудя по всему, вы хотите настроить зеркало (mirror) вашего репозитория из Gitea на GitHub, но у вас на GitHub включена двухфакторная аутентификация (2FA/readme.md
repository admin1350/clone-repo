...
Если встроенная функция зеркал по каким-то причинам не подходит, вы можете настроить "ручное" зеркалирование с помощью хуков Git . Этот метод дает полный контроль, но требует доступа к командной строке сервера.
